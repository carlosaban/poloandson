...
namespace SUIT.Pago.BL
{
    public class BL_Invoice
    {
        ...
        public VE_Invoice updateInvoiceGeneral(VE_Invoice _VeInvoice)
        {
            ...
            if (_VeInvoice.invoiceType != VE_Invoice.PURCHASEORDER &&  ok == 0)
            {
                throw new ApplicationException("Ud. no cuenta con los permisos necesarios");
            }

            if (_VeInvoice.invoiceType != VE_Invoice.PURCHASEORDER && _VeInvoice.documentId != null )
            {
            ...
            /// q es esto dios mio....
            if (_VeInvoice.invoiceType != VE_Invoice.PURCHASEORDER &&  _VeInvoice.invoiceStatusId != invoice[0].invoiceStatusId)
            {
                BL_WorkFlow _blWorkFlow = new BL_WorkFlow();
                _blWorkFlow.connectionString = connectionString;
                var mensaje = "";
